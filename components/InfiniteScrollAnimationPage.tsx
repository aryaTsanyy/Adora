...
const InfiniteScrollAnimationPage = () => {
  useEffect(() => {
    ...
    const container = document.querySelector(".scrolling-text") as HTMLElement;
    let scrollingText = gsap.utils.toArray(".scroll-item") as HTMLElement[];

    // Gandakan elemen teks untuk memastikan looping terus-menerus
    scrollingText.forEach((el) => {
      const clone = el.cloneNode(true) as HTMLElement;
      container.appendChild(clone);
    });
    // Perbarui array elemen dan hitung total width
    const updatedScrollingText = gsap.utils.toArray(".scroll-item") as HTMLElement[];
    const totalWidth = updatedScrollingText.reduce((sum, el: HTMLElement) => sum + el.offsetWidth, 0);

    // Atur posisi elemen
    gsap.set(updatedScrollingText, {
      x: (i) => i * updatedScrollingText[0].offsetWidth,
    });

    // Buat animasi timeline
    const tl = gsap.timeline({
      repeat: -1,
      defaults: { ease: "none" },
    });

    tl.to(updatedScrollingText, {
      x: `-=${totalWidth}`,
      duration: 15,
      ease: "none",
      modifiers: {
  ...
  return (
    <div className="scrolling-text relative overflow-hidden bg-black text-white py-4 flex items-center">
      <div className="scroll-item flex items-center whitespace-nowrap">
